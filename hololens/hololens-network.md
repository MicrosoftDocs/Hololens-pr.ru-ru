...
Для выполнения большинства задач на устройстве HoloLens устройство должно быть подключено к сети. HoloLens содержит радиомодуль Wi-Fi 2x2 с поддержкой стандарта 802.11ac. Его подключение к сети происходит так же, как подключение к сети Wi-Fi классического или мобильного устройства с Windows 10. Это руководство поможет вам:
...
При первом использовании HoloLens вам будет предложено пройти процедуру подключения к сети Wi-Fi. При наличии проблем с подключением к сети Wi-Fi во время настройки проверьте, открыта ли ваша сеть или она защищена паролем и имеет ли она портал авторизации. Кроме того, убедитесь, что для подключения к сети не требуется использование сертификата. После завершения настройки можно будет подключаться к другим типам сетей Wi-Fi.
...
1. В ответ на соответствующий запрос введите сетевой пароль и нажмите **Далее**.
...
> [!TIP]
> Если сеть Wi-Fi недоступна, вы также можете [подключиться к мобильной сети и сети 5G](https://docs.microsoft.com/hololens/hololens-cellular).

> [!IMPORTANT]
> По умолчанию пользователи не могут настроить в HoloLens 2 действие при перемещении в сетях Wi-Fi. **Единственный способ обновить список Wi-Fi — выключить и снова включить сеть Wi-Fi**. Это предотвращает множество проблем, например с зависанием устройства в точке доступа после выхода из ее диапазона.

## <a name="troubleshooting-your-connection-to-wi-fi"></a>Устранение неполадок с подключением к Wi-Fi
...
## <a name="connect-hololens-to-enterprise-wi-fi-network"></a>Подключение HoloLens к сети Wi-Fi предприятия
...
### <a name="eap-troubleshooting"></a>Устранение неполадок EAP
...
### <a name="vpn-via-3rd-party-mdm-solutions"></a>Настройка VPN с помощью решений MDM сторонних производителей.
...
```
## <a name="disabling-wi-fi-on-hololens-1st-gen"></a>Отключение Wi-Fi на HoloLens (1 поколение)

### <a name="using-the-settings-app-on-hololens"></a>С помощью приложения "Параметры" на HoloLens
